    ...
    public class OpenVRController : SensorComponent {
        ...
        public override void StartComponent(Transform trackerTransform) {
            ...
            if (OpenVR.Input != null) {
                ...
                EVRInputError err = OpenVR.Input.GetActionHandle(actionName, ref skeletonActionHandle);
                if (err != EVRInputError.None)
                    Debug.LogError("OpenVR.Input.GetActionHandle error: " + err.ToString());                

                actionName = "/actions/default/out/haptic";
                err = OpenVR.Input.GetActionHandle(actionName, ref hapticsActionHandle);
                if (err != EVRInputError.None) 
                    Debug.LogError("OpenVR.Input.GetActionHandle error: " + err.ToString());
        ...
        public void Vibrate(float length, float strength) {
            //OpenVR.Input.TriggerHapticVibrationAction(hapticsActionHandle, 0, length, 1/length, strength, 0);
            int len = (int)(length * 1000);
            OpenVR.System.TriggerHapticPulse((uint)trackerId, 1, (ushort)len);
        }
