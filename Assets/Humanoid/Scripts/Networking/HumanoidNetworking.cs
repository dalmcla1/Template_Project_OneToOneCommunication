    ...
    public static class HumanoidNetworking {
        ...
        public static void SendAvatarHandPose(this IHumanoidNetworking networking, HandTarget handTarget) {
            ...
            if (handTarget != null) {
                ...
                fingersTarget.thumb.CalculateCurl();
                fingersTarget.index.CalculateCurl();
                fingersTarget.middle.CalculateCurl();
                fingersTarget.ring.CalculateCurl();
                fingersTarget.little.CalculateCurl();
        ...
        public static void ReceiveAvatarPose(
            ...
            if ((targetMask & (1 << 7)) == 0)
                ReceiveTargetWithInterpolationWithoutUpdate(networking, out receivedPosition);
            else
        ...
        private static void ReceiveTargetWithInterpolationWithoutUpdate(
            IHumanoidNetworking networking, out Vector3 receivedPosition) {

            receivedPosition = networking.ReceiveVector3();
            networking.ReceiveQuaternion();
        }
        ...
        private static void ReceiveTargetWithInterpolation(
            ...
            if (deltaPoseTime > 0 && deltaReceiveTime > 0) {
                if (lastReceivedPosition == Vector3.zero)
                    ...
                    //UnityEngine.Debug.Log("Remote V = " + remoteHumanoid.velocity.z);
                }
            }
            else
